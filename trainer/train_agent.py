...
#重复惩罚函数。该函数会在reward里扣分，防止模型生成重复的文本。
# ================================ 工具与 Reward = Start ================================
...
# ======== 模拟执行 ========
MOCK_RESULTS = {
    "calculate_math": lambda args: {"result": str(eval(str(args.get("expression", "0")).replace("^", "**").replace("×", "*").replace("÷", "/").replace("−", "-").replace("（", "(").replace("）", ")"), MATH_GLOBALS))},
...
def parse_tool_calls(text):
    return [
        {"name": result.tool_name, "arguments": result.arguments or {}}
        for result in parse_tool_call_results(text, strict=True)
        if result.ok
    ]
...
def rollout_single(rollout_engine, tokenizer, messages, tools, task=None, max_turns=3, max_new_tokens=256, thinking_ratio=0.5, device="cuda", env_mode="raw", temperature=0.8, top_p=1.0):
    ...
    for turn in range(max_turns):
        ...
        step_info = env.step(new_text)
        if step_info.get("done") or step_info.get("type") != "tool_observation":
            break
        unfinished = turn == max_turns - 1
        if unfinished:
            env.mark_unfinished()

        observe_context = tokenizer.apply_chat_template(env.get_messages(), tokenize=False, add_generation_prompt=not unfinished, tools=env.get_tool_schemas() or tools, open_thinking=open_thinking)
    ...
    if not env.done and unfinished:
        env.mark_unfinished()
    trajectory = env.get_trajectory()
    final_output = trajectory.final_answer or (all_outputs[-1] if all_outputs else "")
...
def rollout_batch(rollout_engine, tokenizer, messages_batch, tools_batch, num_gen, tasks_batch=None, max_turns=3, max_new_tokens=256, thinking_ratio=0.5, device="cuda", env_mode="raw", temperature=0.8, top_p=1.0):
    ...
    return all_completions, all_contexts, all_prompt_ids, all_response_ids, all_response_masks, all_response_old_logps, all_turn_outputs, all_unfinished, all_trajectories
...
def calculate_rewards(prompts, completions, gt_batch, tools_batch, num_gen, reward_model=None, device="cuda", turn_outputs_batch=None, unfinished_batch=None, trajectory_batch=None, tasks_batch=None, return_infos=False):
    ...
    for idx, response in enumerate(completions):
        ...
        if trajectory_batch is not None:
            task = dict(tasks_batch[sample_idx]) if tasks_batch else {}
            task.setdefault("tools", tools_batch[sample_idx])
            task.setdefault("gt", gt_batch[sample_idx])
            if "expected_answer" not in task and gt_batch[sample_idx]:
                task["expected_answer"] = gt_batch[sample_idx]
            reward_value, reward_info = compute_total_reward(trajectory_batch[idx], task)
            rewards[idx] = max(min(float(reward_value), 3.0), -3.0)
        ...
        if not tool_calls:
            ...
            reward_infos.append({"legacy_reward": float(rewards[idx].detach().cpu())})
        # -------- 有工具调用：执行结果奖励 --------
        else:
            ...
            for tool_call in tool_calls:
                ...
                valid_call_count += int(bool(name in valid_names and validate_tool_args(name, raw)))
            ...
            rewards[idx] = max(min(reward, 3.0), -3.0)  # 总分Clip
            reward_infos.append({"legacy_reward": float(rewards[idx].detach().cpu())})
    return (rewards, reward_infos) if return_infos else rewards
...
def rl_train_epoch(epoch, loader, iters, rollout_engine, ref_model, reward_model=None, start_step=0, wandb=None, use_sglang=False):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        if args.loss_type == "cispo":
            ...
            clip_frac = ((ratio > args.epsilon_high) & completion_mask.bool()).float().sum().item() / max(token_counts.sum().item(), 1)
            per_token_loss = -(clamped_ratio * advantages.unsqueeze(1) * per_token_logps - args.beta * per_token_kl)
        else:
            clipped_ratio = torch.clamp(ratio, 1 - args.epsilon, 1 + args.epsilon)
            clip_mask = ((ratio < 1 - args.epsilon) | (ratio > 1 + args.epsilon)) & completion_mask.bool()
            clip_frac = clip_mask.float().sum().item() / max(token_counts.sum().item(), 1)
        ...
        if step % args.log_interval == 0 or step == iters:
            ...
            if gs < args.reward_std_warn_threshold:
                Logger(f'[WARN] Low reward variance: group_reward_std={gs:.8f}; GRPO/CISPO advantage may collapse')
            rb = reward_breakdown
            Logger(
                ...
                f'Loop:{rb.get("loop", 0.0):.3f}, Malformed:{rb.get("malformed", 0.0):.3f}, '
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_weight', default='agent_tool', type=str, help="保存权重名称")
    ...
    parser.add_argument("--data_path", type=str, default="../dataset/agent_rl_tooluse.jsonl", help="训练数据路径")
    ...
    def collate_fn(batch): return {'messages': [b['messages'] for b in batch], 'tools': [b['tools'] for b in batch], 'gt': [b['gt'] for b in batch], 'task': [b.get('task', {}) for b in batch]}
