...
def train_epoch(epoch, loader, iters, teacher_model, lm_config_student, start_step=0, wandb=None, alpha=0.0, temperature=1.0):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        with autocast_ctx:
            res = model(input_ids)
            #取除了最后一个时间步之外的logits，因为第t个位置的输出，是用来预测第t+1个token的
            #.contiguous() 是为了保证内存连续，后面 view() 更安全
        ...
        #如果启用了教师模型，就计算蒸馏项
        if teacher_model is not None:
            #teacher 只推理。不建梯度图省显存
            with torch.no_grad():
                #teacher_logits = [B,T,V]
                teacher_logits = teacher_model(input_ids).logits[..., :-1, :].contiguous()
                #vocab_size_student 表示vocabulary size，即词表大小
                vocab_size_student = student_logits.size(-1)
                #bug：可能出现teacher的词表和student的对不上的可能
        ...
        #loss_mask_flat = [B*(T-1)]
        loss_mask_flat = loss_mask.view(-1)
        #逐token计算交叉熵
        #因为reduction='none'，所以输出每个样本单独的损失值，ce_loss → [B*(T-1)]
...
if __name__ == "__main__":
    ...
    # ====增加student和teacher的模型参数：hidden_size、num_layers、use_moe、from_weight ====
    ...
    #teacher模型不用从ckp恢复状态和编译和分布式包装，因为teacher在这个脚本里只是前向提供软标签，不是训练主体。所以它不需要恢复 optimizer/scaler 这类训练状态；至于 compile 和 DDP，不是“不能”，而是这份实现里作者刻意只把这些优化放在 student 这边
    # ========== 6. 从ckp恢复状态 ==========
