...
class SFTDataset(Dataset):
    def __init__(self, jsonl_path, tokenizer, max_seq_length=1024):
        ...
        self.tokenizer = tokenizer
        self.max_seq_length = max_seq_length
        # Let datasets infer optional top-level fields such as "tools".
    ...
    def create_chat_prompt(self, conversations, sample_tools=None):
        messages = []
        tools = parse_tools_value(sample_tools)
        ...
        for conv in conversations:
            ...
            conv_tools = parse_tools_value(conv.get("tools"))
            if tools is None and conv_tools:
                tools = conv_tools

            # 过滤明显带身份模板的 system
            if role == "system":
                if not conv_tools and any(p.lower() in content.lower() for p in bad_system_phrases):
                    continue
            ...
            if conv.get("reasoning_content"):
                clean_conv["reasoning_content"] = conv.get("reasoning_content")
    ...
    def __getitem__(self, index):
        ...
        sample_tools = sample.get('tools') if hasattr(sample, 'get') else None
        # 先处理结构化对话数据
        ...
        #模板渲染层。创建聊天
        prompt = self.create_chat_prompt(conversations, sample_tools=sample_tools)
        #文本层后处理。对话后处理，删除空思考块
...
#输入是一条完整的对话，输出是messages和tools
    ...
    def parse_task_sample(self, sample):
        tools = parse_tools_value(sample.get("tools")) or get_tool_schemas(sample.get("tools") or [])
        ...
        if expected_answer is None:
            ...
            gt = [str(x) for x in expected_answer]
        else:
            gt = [str(expected_answer)]
        return messages, tools, gt
    ...
    def __getitem__(self, index):
        ...
        if 'conversations' in sample:
            ...
            messages, tools, gt = self.parse_task_sample(sample)
        return {'messages': messages, 'tools': tools, 'gt': gt, 'task': sample}
