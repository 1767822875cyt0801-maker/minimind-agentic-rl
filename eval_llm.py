...
def init_model(args):
    ...
    if 'model' in args.load_from:
        ...
        if args.lora_weight.lower() != 'none':
            apply_lora(model)
...
def main():
    ...
    parser.add_argument('--load_from', default='models', type=str, help="模型加载路径（model=原生torch权重，其他路径=transformers格式）")
    ...
    prompt_iter = prompts if input_mode == 0 else iter(lambda: input('Question：'), '')

    for prompt in prompt_iter:
        setup_seed(random.randint(0, 10000))
        if input_mode == 0:
            print(f'Question：{prompt}')
        conversation = conversations[-args.historys:] if args.historys > 0 else []
        conversation.append({"role": "user", "content": prompt})

        if 'pretrain' in args.weight:
            inputs = tokenizer.bos_token + prompt + tokenizer.eos_token
        else:
        ...
        print(f'[Speed: {gen_tokens / (time.time() - start_time):.2f} tokens/s]') if args.show_speed else print('\n\n')
